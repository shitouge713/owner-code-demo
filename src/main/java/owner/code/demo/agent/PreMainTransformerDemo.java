...
public class PreMainTransformerDemo implements ClassFileTransformer {
    ...
    public byte[] transform(ClassLoader loader,
        ...
        // className 默认格式 owner/code/demo/agent 替换 owner.code.demo.agent
        className = className.replace("/", ".");
        //java自带的方法不进行处理,不是特别类的方法也不处理
        if (className.startsWith("java") || className.startsWith("sun") || !className.contains("owner.code.demo.agent")) {
            return null;
        }
        CtClass ctclass;
        try {
            ...
            for (CtMethod ctMethod : ctclass.getDeclaredMethods()) {
                ...
                //方法完成后，运算方法执行时间
                bodyStr.append("System.out.println(\"==============退出 Method: " + className + "." + methodName + " Cost:\" +(endTime - startTime) +\"ms " + "\");");
                bodyStr.append("}");
                // 新方法字节码替换原来的方法字节码
